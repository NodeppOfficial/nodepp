...
namespace nodepp { namespace _ws_ {
    ...
    private:
        ...
        ulong size=0, len=0, state=0, sz=0;
        ws_frame_t frame;
    ...
    gnStart;
        ...
        memset( bf, 0, sx ); size = 0; data = 0; len=0;
         while( read_ws_frame(bf,size,len)==1 ){ len=0;
        coWait( str->_read_( bf, size, len ) );} len=0;
        ...
        do{ for( auto x=state; x-->0; ){
            bf[x]=bf[x]^frame.KEY[len]; len=(len+1) % 4;
        } } while(0);
