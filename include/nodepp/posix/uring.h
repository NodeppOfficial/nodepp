...
protected:
    ...
    struct PAIR { void* addr; DONE* ctx   ; };
    struct NODE {
        ...
        IOpll  pqes  ; IOprm  prm;
        void  *sq_ptr, *cq_ptr; 

        uint entries ; int batch;

        queue_t<PAIR > borrow;
        queue_t<DENO > que   ; int ed;
        queue_t<DONE*> sque  ; int fd;
    ...
    queue_t<void*> out ;
    while( out.size() < NODEPP_MAX_BATCH_SIZE ){
    if( obj->borrow.last() == nullptr ) /*--*/ { break; } do {
        auto &data = obj->borrow.last()->data;
    if( data.ctx->mode!=FLAG::URING_FLAG_DONE ){ break; }
        out.push( data.addr );
    } while(0); obj->borrow.pop(); } return out.data(); }
...
protected:
    ...
    int  z=0; auto x=obj->sque.first();
        ...
        while( tail - head < obj->entries ){
        if   ( x==nullptr )/*--*/{ break; }

            uint index=tail&*obj->sq_mask; tail++; z++;
            obj->sqes[index] = x->data->sqe; x=x->next;
    ...
    do { if( *obj->cq_head == *obj->cq_tail ) { break; } int z=0; 
    uint head = NODEPP_URING_ATOMIC_LOAD( obj->cq_head );
        
        while( head != *obj->cq_tail ){ 
        
            uint  index = head & *obj->cq_mask;
            IOcqe  *cqe = &obj->cqes[index];  

            uchar_64 pt = cqe->user_data &  0x0000FFFFFFFFFFFF;
            uint   mode = cqe->user_data >> 48;

            auto y = obj->que.as( (void*) pt );
            if ( y != nullptr ) { do {

                y->data.data[mode].mode = FLAG::URING_FLAG_DONE;
                y->data.data[mode].cqe  = *cqe; z++;

            if( y->data.addr==nullptr ){ break; }
                obj->borrow.unshift( PAIR({ y->data.addr, &y->data.data[mode] }) );
            } while(0); }
        
        head++; }

    NODEPP_URING_ATOMIC_SAVE( obj->cq_head, head ); 
    return z; } while(0); return -1; }

public:

    uring_t() : obj( new NODE() ) { 
    ...
    return ::write(obj->ed,&value,sizeof(uint64_t)); }
    ...
    ptr_t<void*> next( int ms ) const noexcept {
        ...
        if( ms!=0&&obj->borrow.empty() ){ uint64_t value=0;
            poll( &obj->pqes, 1, ms ); io_consume();
        if( ::read( obj->ed,&value,sizeof(uint64_t)) ){} }
